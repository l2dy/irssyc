...
sub get_context {
  ...
  unless (ref $vars eq 'HASH') {
    ...
    $addr = $vars->{_context} || $vars->{_source_relay} || $vars->{_source};
  }
...
sub msg {
  ...
  if ($func) {
    $source = $vars->{_source_relay} || $vars->{_source} if $source eq UNI();
    ...
    unless (ref $c) {
	    ...
	    W0('Client->msg(%s, %s, %s, %s) is baklawa.', $source, $mc, $data, $vars);
      print "z0mg!!!!!!!!!!!!!!!!!!!!";
      #print Dumper($vars);
	    return 1;
    }
...
sub psycLink {
  ...
  Net::PSYC::sendmsg($UNI, '_request_link', '',
                     { _password => $pass || main::getPassword(),
                     });
...
%react = (
          ...
          '_notice_link'		=> sub {
            ...
            if (same_host($user[1], $uni[1]) && lc($user[4]) eq lc($uni[4])) {
              ...
              W0('Got a _notice_link from an uni (%s) we did not try to link to. ['.$source.']');
            }
          },
